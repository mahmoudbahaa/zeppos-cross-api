...
export class BloodOxygen {
  constructor() {
    this._spo2 = hmSensor.createSensor(hmSensor.id.SPO2);
  }
   ...
   */
  getCurrent() {
    return { value: this._spo2.current, time: this._spo2.time, retCode: this._spo2.time.retcode };
  }
   ...
   */
  getLastDay() {
    return this._spo2.hourAvgofDay;
  }
   ...
   */
  onChange(callback) {
    this._spo2.addEventListener(hmSensor.event.CHANGE, callback);
  }
   ...
   */
  offChange(callback) {
    this._spo2.removeEventListener(hmSensor.event.CHANGE, callback);
  }
