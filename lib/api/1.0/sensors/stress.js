...
export class Stress {
  constructor() {
    this._stress = hmSensor.createSensor(hmSensor.id.STRESS);
  }
   ...
   */
  getCurrent() {
    return { value: this._stress.current, time: this._stress.time };
  }
   ...
   */
  onChange(callback) {
    this._stress.addEventListener(hmSensor.event.CHANGE, callback);
  }
   ...
   */
  offChange(callback) {
    this._stress.removeEventListener(hmSensor.event.CHANGE, callback);
  }
