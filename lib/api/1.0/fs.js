/* global hmFS */
import { bin2str, str2bin } from './zml/common/data';
...
export const readFileSync = option => {
  ...
  return bin2str(buffer, option.options.encoding);
};

export const writeFileSync = option => {
  const fd = typeof option.path === 'number'
    ? option.path
    : hmFS.open(option.path, hmFS.O_WRONLY || hmFS.O_CREAT);

  const buffer = typeof option.data === 'string'
    ? str2bin(option.data, option.options && option.options.encoding)
