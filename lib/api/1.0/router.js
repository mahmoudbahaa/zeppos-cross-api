...
export const launchApp = option => hmApp.startApp({
  appid: option.appId, url: option.url, native: option.native, param: option.params,
});
export const push = option => hmApp.gotoPage({ url: option.url, param: option.params === undefined ? undefined : (typeof option.params === 'string' ? option.params : JSON.stringify(option.params)) });
export const replace = option => hmApp.reloadPage({ url: option.url, param: option.params === undefined ? undefined : (typeof option.params === 'string' ? option.params : JSON.stringify(option.params)) });
export const home = () => hmApp.gotoHome();
export const back = () => hmApp.goBack();
export const exit = () => hmApp.exit();
export const setLaunchAppTimeout = option => hmApp.alarmNew({
