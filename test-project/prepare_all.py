...
def createProject(APi_LEVEL):
  ...
  for fileOrFolder in sources:
    if Path(fileOrFolder).is_dir():
      shutil.copytree(fileOrFolder, APi_LEVEL + "/" + fileOrFolder)
    else:
      shutil.copy(fileOrFolder, APi_LEVEL + "/" + fileOrFolder)

  shutil.copytree("../dist/" + APi_LEVEL, APi_LEVEL + "/lib/api")
